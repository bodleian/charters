...
if [ ! "$5" == "noindex" ]; then
    ...
    curl -X POST -fsS "http://${4}:8983/solr/charters-mss/update?commit=true" --data-binary "<delete><query>type:${3}</query></delete>" -H "Content-Type: text/xml" 1>> $LOGFILE 2>> $LOGFILE
    ...
    if [ $? -gt 0 ]; then
        ...
        curl -fsS "http://${4}:8983/solr/charters-mss/update?commit=true" --data-binary @solr/$2 -H "Content-Type: text/xml" 1>> $LOGFILE 2>> $LOGFILE
        if [ $? -eq 0 ]; then

...
if [ "$1" == "reuse" ]; then
    ...
    if [ $ageofsolrdir -ge 1 ]; then
        ...
        while true; do
            read -r -p "Are you sure you want to send these to the localhost Solr server?? [Yes|No] " answer
            case $answer in
                [Yy]|YES|Yes|yes ) break;;
                [Nn]|NO|No|no ) echo "Abandoning re-indexing. The localhost server has not been updated."; exit 0;;
                * ) echo;;
            esac
        done
    fi
    ...
    if [ "$1" == "force" ] || [ "$1" == "noindex" ]; then
        ...
        ./generate-solr-document.sh "collections.xquery" "collections_index.xml" collection $SERVER "$1"
    fi
fi

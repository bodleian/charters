...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    ...
    <xsl:include href="customizationsMinimal.xsl"/>
    ...
    <xsl:param name="collection" select="'D_D_Ch_Ch_D'"></xsl:param>
    <xsl:param name="recurse" select="'yes'"/>
    <xsl:variable name="path">
        <xsl:value-of
            select="concat('../tei-qa/', $collection, '?select=', $files,';on-error=warning;recurse=',$recurse)"
        />
    </xsl:variable>
